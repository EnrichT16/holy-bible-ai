  ...
  return (
    <Screen edges={['top', 'bottom']}>
      <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
        ...
        {pane === 1 && (
          <View style={styles.pane}>
            ...
            <Text style={styles.title} accessibilityRole="header" aria-level={1}>Choose your Bible</Text>
            ...
            <Card style={{ marginTop: 18, alignSelf: 'stretch' }}>
              {VERSIONS.map((v) => {
                ...
                return (
                  ...
                  >
                    <View style={styles.badge} aria-hidden><Text style={styles.badgeText}>{v.abbrev}</Text></View>
                    <View style={{ flex: 1 }}>
        ...
        {pane === 2 && (
          <View style={styles.pane}>
            ...
            <Text style={styles.title} accessibilityRole="header" aria-level={1}>Choose your light</Text>
            ...
            <Card style={{ marginTop: 18, alignSelf: 'stretch' }}>
              <Pressable
                ...
                onPress={clearPin}
                accessibilityRole="button"
                accessibilityLabel="Rotate the theme daily"
      ...
      <View style={styles.controls}>
        {pane > 0 ? (
          <Pressable style={styles.back} onPress={() => setPane((p) => p - 1)} accessibilityRole="button" accessibilityLabel="Back to the previous step">
            <Ionicons name="chevron-back" size={22} color={Lumen.colors.text} aria-hidden />
          </Pressable>
        ) : (
          <Pressable style={styles.skip} onPress={finish} accessibilityRole="button" accessibilityLabel="Skip the welcome">
            <Text style={styles.skipText}>Skip</Text>
          </Pressable>
        )}
        <Pressable
          style={styles.next}
          onPress={() => (pane < 2 ? setPane((p) => p + 1) : finish())}
          accessibilityRole="button"
          accessibilityLabel={pane < 2 ? 'Continue to the next step' : 'Begin reading'}
        >
      ...
      <View style={styles.dots} aria-hidden accessible={false} importantForAccessibility="no-hide-descendants">
        {[0, 1, 2].map((i) => (
...
const styles = StyleSheet.create({
  ...
  back: { width: 52, minHeight: 52, borderRadius: 26, alignItems: 'center', justifyContent: 'center', backgroundColor: Lumen.colors.card, borderWidth: 1, borderColor: Lumen.colors.cardBorder },
  skip: { width: 72, minHeight: 52, borderRadius: 26, alignItems: 'center', justifyContent: 'center' },
  skipText: { fontFamily: Lumen.fonts.body, color: Lumen.colors.muted, fontSize: 14 },
  next: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, minHeight: 52, borderRadius: 26, backgroundColor: Lumen.colors.accent },
